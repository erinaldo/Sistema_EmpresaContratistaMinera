...
{
    public partial class FormResumenAsientoVenta : Biblioteca.Formularios.FormBase
    {
        #region Atributos
        private N_ResumenAsientoVenta nResumenAsientoVenta = new N_ResumenAsientoVenta();
        private List<CatalogoBase> _lista = new List<CatalogoBase>();
        private ContextMenu menuContextual = new ContextMenu();
        #endregion

        public FormResumenAsientoVenta()
        {
            InitializeComponent();
        }

        #region Eventos de Formulario
        private void FormResumenAsientoVenta_Load(object sender, EventArgs e)
        {
        ...
        private void cargarResumenAsientoVenta()
        {
            string periodo = cmbPeriodo.Text + "-" + Convert.ToString(txtPeriodo.Value);
            _lista = nResumenAsientoVenta.obtenerCatalago(periodo); //Carga los Asientos Contables
            lstListado.DataSource = _lista;
            lstListado.ValueMember = "Id";
            lstListado.DisplayMember = "Denominacion";
            calcularTotal(nResumenAsientoVenta.contabilizarDebeHaber(periodo)); //Calcula el total del Debe y Haber del Resumen
        }
        ...
        private void reportarResumenAsientoVenta()
        {
            if (lstListado.Items.Count > 0)
            {
                Cursor.Current = Cursors.WaitCursor;
                string periodo = cmbPeriodo.Text + "-" + Convert.ToString(txtPeriodo.Value);
                _lista = nResumenAsientoVenta.obtenerCatalago(periodo, "CATALOGO2");
                ...
                string tituloA = "Resumen de Asientos de Venta";
