...
{
    public partial class FormEstadoFinanciero : Biblioteca.Formularios.FormBase
    {
        #region Atributos
        private N_EstadoFinanciero nEstadoFinanciero = new N_EstadoFinanciero();
        private List<CatalogoBase> _lista = new List<CatalogoBase>();
        private ContextMenu menuContextual = new ContextMenu();
        #endregion

        public FormEstadoFinanciero()
        {
            InitializeComponent();
        }

        #region Eventos de Formulario
        private void FormEstadoFinanciero_Load(object sender, EventArgs e)
        {
        ...
        private void cargarEstadoFinanciero()
        {
            string periodo = cmbPeriodo.Text + "-" + Convert.ToString(txtPeriodo.Value);
            _lista = nEstadoFinanciero.obtenerCatalago(periodo); //Carga los Asientos Contables
        ...
        private void reportarEstadoFinanciero()
        {
            if (lstListado.Items.Count > 0)
            {
                Cursor.Current = Cursors.WaitCursor;
                string periodo = cmbPeriodo.Text + "-" + Convert.ToString(txtPeriodo.Value);
                _lista = nEstadoFinanciero.obtenerCatalago(periodo, "CATALOGO2"); //Carga los Asientos Contables
                ...
                string tituloA = "Estado Financiero";
