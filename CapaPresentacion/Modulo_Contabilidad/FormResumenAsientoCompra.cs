...
{
    public partial class FormResumenAsientoCompra : Biblioteca.Formularios.FormBase
    {
        #region Atributos
        private N_ResumenAsientoCompra nResumenAsientoCompra = new N_ResumenAsientoCompra();
        private List<CatalogoBase> _lista = new List<CatalogoBase>();
        private ContextMenu menuContextual = new ContextMenu();
        #endregion

        public FormResumenAsientoCompra()
        {
            InitializeComponent();
        }

        #region Eventos de Formulario
        private void FormResumenAsientoCompra_Load(object sender, EventArgs e)
        {
        ...
        private void cargarResumenAsientoCompra()
        {
            string periodo = cmbPeriodo.Text + "-" + Convert.ToString(txtPeriodo.Value);
            _lista = nResumenAsientoCompra.obtenerCatalago(periodo); //Carga los Asientos Contables
            lstListado.DataSource = _lista;
            lstListado.ValueMember = "Id";
            lstListado.DisplayMember = "Denominacion";
            calcularTotal(nResumenAsientoCompra.contabilizarDebeHaber(periodo)); //Calcula el total del Debe y Haber del Resumen
        }
        ...
        private void reportarResumenAsientoCompra()
        {
            if (lstListado.Items.Count > 0)
            {
                Cursor.Current = Cursors.WaitCursor;
                string periodo = cmbPeriodo.Text + "-" + Convert.ToString(txtPeriodo.Value);
                _lista = nResumenAsientoCompra.obtenerCatalago(periodo, "CATALOGO2");
                ...
                string tituloA = "Resumen de Asientos de Compra";
