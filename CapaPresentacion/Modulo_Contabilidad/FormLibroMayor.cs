...
{
    public partial class FormLibroMayor : Biblioteca.Formularios.FormBase
    {
        #region Atributos
        private long _idCuentaContable = 0;
        private CuentaContable _cuentaContable = new CuentaContable();
        private N_LibroMayor nLibroMayor = new N_LibroMayor();
        ...
        private void cargarLibroMayor()
        {
            ...
            _lista = nLibroMayor.obtenerCatalago(_idCuentaContable, pkrPeriodoDesde.Value, pkrPeriodoHasta.Value); //Carga los Asientos Contables
            lstListado.DataSource = _lista;
            lstListado.ValueMember = "Id";
            lstListado.DisplayMember = "Denominacion";
            calcularTotal(nLibroMayor.contabilizarDebeHaber(_idCuentaContable, pkrPeriodoDesde.Value, pkrPeriodoHasta.Value)); //Calcula el total del Debe y Haber del Libro Mayor
        }
        ...
        private void copiarTextoelemento(object sender, EventArgs e)
        {
            ...
            string texto = "Cuenta Contable: " + columna[0].Trim() + "; Fecha: " + columna[1].Trim() + "; " + columna[2].Trim() + "; Debe $" + columna[3].Trim() + "; Haber $" + columna[4].Trim() + "; Saldo $" + columna[5].Trim();
        ...
        private void reportarLibroMayor()
        {
            if (lstListado.Items.Count > 0)
            {
                Cursor.Current = Cursors.WaitCursor;
                _lista = nLibroMayor.obtenerCatalago(_idCuentaContable, pkrPeriodoDesde.Value, pkrPeriodoHasta.Value, "CATALOGO2");
                ...
                string tituloA = "Libro Mayor";
                string tituloB = "Periodo " + pkrPeriodoDesde.Text.Substring(0, 10).Replace("/", "-") + " Al " + pkrPeriodoHasta.Text.Substring(0, 10).Replace("/", "-");
                reporte.crearDocumentoExcel_ListaConTotal(tituloA, tituloB, subTitulos, new int[] { 25, 12, 56, 12, 12, 12 }, _listaDelReporte, new List<int> { 1 }, leyendasDeTotal, valoresDeTotal, 2, 1, 115, 100, true);
            }
        }
        #endregion
    }
