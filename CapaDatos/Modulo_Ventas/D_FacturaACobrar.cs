...
namespace CapaDatos
{
    public class D_FacturaACobrar : IFacturaACobrar, IDisposable
    {
        #region Atributos
        ...
        private const string SELECT = @"SELECT data_asiento_contable.id AS id_asiento, asiento_nro, asiento_fecha,
            ...
            data_cliente.denominacion AS proveedor, cuit, data_venta.cobranza_estado";
        private const string FROM = @" FROM data_asiento_contable 
            LEFT JOIN data_venta ON origen_tipo = 'CPR' AND origen_id = data_venta.id AND data_venta.cobranza_estado <> 'COBRADO'
                AND (data_venta.afip_cbte_tipo = 01
                    OR data_venta.afip_cbte_tipo = 06
                    OR data_venta.afip_cbte_tipo = 11
                    OR data_venta.afip_cbte_tipo = 51
                    OR data_venta.afip_cbte_tipo = 02
                    OR data_venta.afip_cbte_tipo = 07
                    OR data_venta.afip_cbte_tipo = 12
                    OR data_venta.afip_cbte_tipo = 52
                    OR data_venta.afip_cbte_tipo = 01)
            INNER JOIN data_cliente ON data_venta.id_cliente = data_cliente.id";
        private const string WHERE1 = @" WHERE data_asiento_contable.id_cuenta_contable = 6 AND (date(data_asiento_contable.asiento_fecha) >= date(@desde) AND date(data_asiento_contable.asiento_fecha) <= date(@hasta))"; //Filtro por Objeto Cuenta Contable (DEUDORES POR VENTA) y Fecha de Asiento
        private const string WHERE2 = @" WHERE data_asiento_contable.id_cuenta_contable = 6 AND (date(data_asiento_contable.asiento_fecha) >= date(@desde) AND date(data_asiento_contable.asiento_fecha) <= date(@hasta)) AND data_cliente.cuit = @cuit"; //Filtro por Objeto Cuenta Contable (DEUDORES POR VENTA), Fecha de Asiento y CUIT
        ...
        public List<CatalogoBase> obtenerCatalago(string campoEspecifico, string valorEspecifico, DateTime desde, DateTime hasta, string catalogo, int indicePagina, int tamanioPagina)
        {
            ...
            List<FacturaACobrar> ListaDeObjetos = new List<FacturaACobrar>(); //Crea una lista de Objetos para almacenar los registros de la tabla
            try
            {
                if (_conexion.crearConexion()) //Verifica la creación de la conexión con el Servidor de Base de Datos
                {
                    ...
                    using (MySqlCommand comandoDB = _conexion.crearComandoDB(SELECT + FROM + condicional + ORDER + paginacion)) //Crea un comando de Base de Datos
                    {
                        ...
                        using (MySqlDataReader lectorDB = comandoDB.ExecuteReader())
                        {
                            while (lectorDB.Read())
                            {
                                FacturaACobrar objFacturaACobrar = instanciarObjeto(lectorDB); //Crea un objeto y asigna los valores correspondientes.
                                ListaDeObjetos.Add(objFacturaACobrar); //Agrega este Objeto a la lista de Objetos
                            }
                        }
                    }
                    foreach (FacturaACobrar itemFacturaACobrar in ListaDeObjetos)
                    {
                        if (catalogo == "CATALOGO1")
                        {
                            CatalogoBase elemento = new CatalogoBase(
                                 itemFacturaACobrar.Id,
                                 Convert.ToInt64(itemFacturaACobrar.Cuit).ToString("00-00000000/0") +
                                    " | " + Fecha.ConvertirFecha(itemFacturaACobrar.AsientoFecha).PadLeft(10, '0') +
                                    " | " + itemFacturaACobrar.Descripcion.PadRight(44, ' ') + //Establecí el valor del ancho de esta columna en 44 para rellenar el ListBox
                                    " | " + Formulario.ValidarCampoMoneda(itemFacturaACobrar.Debe).PadLeft(12, ' ') +
                                    " | " + (itemFacturaACobrar.EstadoCobro).PadRight(8, ' '));
                        ...
                        else if (catalogo == "CATALOGO2")
                        {
                            ...
                            if (cuitAntecesor != itemFacturaACobrar.Cuit)
                            {
                                double[] Totales = contabilizarDebeHaber("CUIT", itemFacturaACobrar.Cuit, desde, hasta);
                                elemento = new CatalogoBase(
                                     -1, //ID nulo
                                     itemFacturaACobrar.Denominacion.PadRight(35, ' ') +
                                        " | " + Convert.ToInt64(itemFacturaACobrar.Cuit).ToString("00-00000000/0").PadRight(13, ' ') +
                                        " | " + ("").PadLeft(10, ' ') +
                        ...
                        cuitAntecesor = itemFacturaACobrar.Cuit; //Deducción de Saldo Antecesor - Paso 2: Establece el CUIT actual (Importante para completar el ciclo)
                    }
                }
            }
        ...
        private FacturaACobrar instanciarObjeto(MySqlDataReader lectorDB) //Método que crea una instancia del Objeto con información de la Base de datos
        {
            return new FacturaACobrar(
