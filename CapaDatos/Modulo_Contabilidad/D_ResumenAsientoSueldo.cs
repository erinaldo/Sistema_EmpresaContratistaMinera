...
namespace CapaDatos
{
    public class D_ResumenAsientoSueldo : IResumenAsientoSueldo, IDisposable
    {
        #region Atributos
        ...
        private const string WHERE = @" WHERE origen_tipo = 'LQS' AND
            (LOWER(data_cuenta_contable.tipo_cuenta) LIKE LOWER('%EGRESOS > COSTO DE NOMINA%') 
            OR LOWER(data_cuenta_contable.tipo_cuenta) LIKE LOWER('%PASIVO CORRIENTE > DEUDAS SOCIALES%')) 
        ...
        private const string ORDER = @" ORDER BY data_cuenta_contable.id ASC";
        ...
        public List<CatalogoBase> obtenerCatalago(string periodo, string catalogo, int indicePagina, int tamanioPagina)
        {
            ...
            List<ResumenAsientoSueldo> ListaDeObjetos = new List<ResumenAsientoSueldo>(); //Crea una lista de Objetos para almacenar los registros de la tabla
            try
            {
                if (_conexion.crearConexion()) //Verifica la creación de la conexión con el Servidor de Base de Datos
                {
                    ...
                    using (MySqlCommand comandoDB = _conexion.crearComandoDB(SELECT + FROM + WHERE + GROUP + ORDER + paginacion)) //Crea un comando de Base de Datos
                    {
                        ...
                        using (MySqlDataReader lectorDB = comandoDB.ExecuteReader())
                        {
                            while (lectorDB.Read())
                            {
                                ResumenAsientoSueldo objResumenAsientoSueldo = instanciarObjeto(lectorDB); //Crea un objeto y asigna los valores correspondientes.
                                ListaDeObjetos.Add(objResumenAsientoSueldo); //Agrega este Objeto a la lista de Objetos
                            }
                        }
                    }
                    foreach (ResumenAsientoSueldo itemResumenAsientoSueldo in ListaDeObjetos)
                    {
                        if (catalogo == "CATALOGO1")
                        {
                            CatalogoBase elemento = new CatalogoBase(
                                itemResumenAsientoSueldo.CuentaContable.Id,
                                itemResumenAsientoSueldo.CuentaContable.Denominacion.PadRight(88, ' ') + //Asigné el valor en 85 para cubrir el ancho del ListBox 
                                    " | " + Formulario.ValidarCampoMoneda(itemResumenAsientoSueldo.Debe).PadLeft(12, ' ') +
                                    " | " + Formulario.ValidarCampoMoneda(itemResumenAsientoSueldo.Haber).PadLeft(12, ' '));
                        ...
                        else if (catalogo == "CATALOGO2")
                        {
                            CatalogoBase elemento = new CatalogoBase(
                                itemResumenAsientoSueldo.CuentaContable.Id,
                                itemResumenAsientoSueldo.CuentaContable.Denominacion.PadRight(25, ' ') +
                                    " | " + Formulario.ValidarCampoMoneda(itemResumenAsientoSueldo.Debe).PadLeft(12, ' ') +
                                    " | " + Formulario.ValidarCampoMoneda(itemResumenAsientoSueldo.Haber).PadLeft(12, ' '));
                            ListaDeElementos.Add(elemento); //Agrega este elemento a la lista de elementos
                        }
            ...
            catch (MySqlException e) { Mensaje.Error("Error-M002RESUMEN_SUELDO: Hay un conflicto en la consulta de la cuenta contable.", e); }
        ...
        public double[] contabilizarDebeHaber(string periodo)
        {
            ...
            catch (MySqlException e) { Mensaje.Error("Error-M004RESUMEN_SUELDO: Hay un conflicto en el cálculo del debe y haber del balance de sumas y saldos .", e); }
            finally { _conexion.Dispose(); }
        ...
        private ResumenAsientoSueldo instanciarObjeto(MySqlDataReader lectorDB) //Método que crea una instancia del Objeto con información de la Base de datos
        {
            return new ResumenAsientoSueldo(
