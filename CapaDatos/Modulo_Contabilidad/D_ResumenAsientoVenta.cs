...
namespace CapaDatos
{
    public class D_ResumenAsientoVenta : IResumenAsientoVenta, IDisposable
    {
        #region Atributos
        ...
        private const string WHERE = @" WHERE (LOWER(data_cuenta_contable.tipo_cuenta) LIKE LOWER('%INGRESOS > INGRESO POR VENTAS%')
            OR data_cuenta_contable.id = 17 OR data_cuenta_contable.id = 6)
        ...
        private const string ORDER = @" ORDER BY (data_cuenta_contable.id = 6), data_cuenta_contable.denominacion DESC"; //Ordena por nombre de Cuenta Contable de manera descendente y coloca al final de la lista la Cuenta Contable "DEUDORES POR VENTA"
        ...
        public List<CatalogoBase> obtenerCatalago(string periodo, string catalogo, int indicePagina, int tamanioPagina)
        {
            ...
            List<ResumenAsientoVenta> ListaDeObjetos = new List<ResumenAsientoVenta>(); //Crea una lista de Objetos para almacenar los registros de la tabla
            try
            {
                if (_conexion.crearConexion()) //Verifica la creación de la conexión con el Servidor de Base de Datos
                {
                    ...
                    using (MySqlCommand comandoDB = _conexion.crearComandoDB(SELECT + FROM + WHERE + GROUP + ORDER + paginacion)) //Crea un comando de Base de Datos
                    {
                        ...
                        using (MySqlDataReader lectorDB = comandoDB.ExecuteReader())
                        {
                            while (lectorDB.Read())
                            {
                                ResumenAsientoVenta objResumenAsientoVenta = instanciarObjeto(lectorDB); //Crea un objeto y asigna los valores correspondientes.
                                ListaDeObjetos.Add(objResumenAsientoVenta); //Agrega este Objeto a la lista de Objetos
                            }
                        }
                    }
                    foreach (ResumenAsientoVenta itemResumenAsientoVenta in ListaDeObjetos)
                    {
                        if (catalogo == "CATALOGO1")
                        {
                            CatalogoBase elemento = new CatalogoBase(
                                itemResumenAsientoVenta.CuentaContable.Id,
                                itemResumenAsientoVenta.CuentaContable.Denominacion.PadRight(88, ' ') + //Asigné el valor en 85 para cubrir el ancho del ListBox 
                                    " | " + Formulario.ValidarCampoMoneda(itemResumenAsientoVenta.Debe).PadLeft(12, ' ') +
                                    " | " + Formulario.ValidarCampoMoneda(itemResumenAsientoVenta.Haber).PadLeft(12, ' '));
                        ...
                        else if (catalogo == "CATALOGO2")
                        {
                            CatalogoBase elemento = new CatalogoBase(
                                itemResumenAsientoVenta.CuentaContable.Id,
                                itemResumenAsientoVenta.CuentaContable.Denominacion.PadRight(25, ' ') +
                                    " | " + Formulario.ValidarCampoMoneda(itemResumenAsientoVenta.Debe).PadLeft(12, ' ') +
                                    " | " + Formulario.ValidarCampoMoneda(itemResumenAsientoVenta.Haber).PadLeft(12, ' '));
                            ListaDeElementos.Add(elemento); //Agrega este elemento a la lista de elementos
                        }
            ...
            catch (MySqlException e) { Mensaje.Error("Error-M002RESUMEN_VENTA: Hay un conflicto en la consulta de la cuenta contable.", e); }
        ...
        public double[] contabilizarDebeHaber(string periodo)
        {
            ...
            catch (MySqlException e) { Mensaje.Error("Error-M004RESUMEN_VENTA: Hay un conflicto en el cálculo del debe y haber del balance de sumas y saldos .", e); }
            finally { _conexion.Dispose(); }
        ...
        private ResumenAsientoVenta instanciarObjeto(MySqlDataReader lectorDB) //Método que crea una instancia del Objeto con información de la Base de datos
        {
            return new ResumenAsientoVenta(
